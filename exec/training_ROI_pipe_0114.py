...
for seg_id in seg_ids:
    models_saving_paths[seg_id] = os.path.join(repo_root, f"models/ROI/ROI_{date}_{m_id}_{seg_id}.json")
...
for seg_id in seg_ids:
    ...
    W_train = add_vanilla_weights(ROI_train, "label_same_user_st").to_numpy(dtype="float64")
    W_test = add_vanilla_weights(ROI_test, "label_same_user_st").to_numpy(dtype="float64")

    train_pool = cat.Pool(
        data=X_train,
        label=y_train,
        cat_features=["Weekday_marker"],
        weight=W_train)
    ...
    eval_result = model.eval_metrics(train_pool, metrics=["Logloss"])

    # save the model:
    model.save_model(
        models_saving_paths[seg_id],
        format="json",
        pool=train_pool
    )
    ...
    y_pred_insample_raw = model.predict(X_train, 
                                    prediction_type="RawFormulaVal")
    y_pred_insample = sigmoid(y_pred_insample_raw)

    y_pred_test_raw = model.predict(X_test,
                                prediction_type="RawFormulaVal")
    y_pred_test = sigmoid(y_pred_test_raw)
